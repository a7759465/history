...
use app\admin\library\Lunar;
use app\admin\library\LunarSolarConverter;
use app\admin\library\Solar;
...
if (!function_exists('SolarToLunar')) {
    function SolarToLunar($year, $month, $day)
    {
        require_once(dirname(__FILE__) . '/library/LunarSolarConverter.class.php');
        $solar = new Solar();
        $solar->solarYear = $year;
        $solar->solarMonth = $month;
        $solar->solarDay = $day;
        $lunar = LunarSolarConverter::SolarToLunar($solar);
        $time = mktime(0, 0, 0, $lunar->lunarMonth, $lunar->lunarDay, $lunar->lunarYear);
        $lunar->text = date('Y-m-d H:i:s', $time);

        if ($lunar->isleap) {
            $lunar->text_date_cn = date('Y年闰m月d日', $time);
            $lunar->text_datetime_cn = date('Y年闰m月d日 H时i分s秒', $time);
        } else {
            $lunar->text_date_cn = date('Y年闰m月d日', $time);
            $lunar->text_datetime_cn = date('Y年闰m月d日 H时i分s秒', $time);
        }
...
if (!function_exists('LunarToSolar')) {
    function LunarToSolar($year, $month, $day, $isLeap = false)
    {
        ...
        $time = mktime(0, 0, 0, $solar->solarMonth, $solar->solarDay, $solar->solarYear);

        $solar->text = date('Y-m-d H:i:s', $time);
        $solar->text_date_cn = date('Y年m月d日', $time);
        $solar->text_datetime_cn = date('Y年m月d日 H时i分s秒', $time);
        return $solar;
    }
}
